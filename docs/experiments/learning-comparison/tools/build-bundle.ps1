...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $RepositoryRoot ".tempo\experiments\learning-comparison\bundle"
}
...
if ([string]::IsNullOrWhiteSpace($ProofVideo)) {
    $ProofVideo = Join-Path $RepositoryRoot ".tempo\experiments\graph-video-spike\proof.mp4"
}
