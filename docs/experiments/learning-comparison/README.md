...
`.understand-video/experiments/learning-comparison/bundle/` directory. The two short video
...
`.understand-video/experiments/learning-comparison/observations/`. Otherwise click
...
## Compile real observations into TEMPO-compatible evidence proposals
...
powershell -NoProfile -ExecutionPolicy Bypass -File `
  ...
  -InputDirectory .understand-video/experiments/learning-comparison/observations `
  -OutputDirectory .understand-video/experiments/learning-comparison/evidence
```
...
After human review, move to the **separate** TEMPO checkout pinned by
[`TEMPO_BASELINE.json`](../../../TEMPO_BASELINE.json), add every generated
evidence item there, and rerun readiness. Do not omit unfavorable participants
and do not copy the TEMPO CLI into this repository:

```powershell
cd <separate-tempo-checkout>
python bin/tempo evidence add --input <reviewed-generated-evidence-file>
